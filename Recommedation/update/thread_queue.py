...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        elif self.work[0] == 'update_follow_count':
            update_follow_count(self.name, self.queue,self.work[1])
...
def update_follow_count(thread_name, queue, table):
    while not EXIT_FLAG:
        QUEUE_LOCK.acquire()
        if not WORK_QUEUE.empty():
            try:
                ...
                if html_data[0]!=-1:
                    result = html_analysis.get_follow_count(html_data[1])
                if result[0] != -1:
                    follow_count = result[1]
                    print("%s: follow_count %d" % (thread_name, follow_count))
                    sql = 'update ' + table + ' set update_shop_time=%s,follow_count=%s where shop_id=%s '
                    data = [datetime.datetime.now(),follow_count, shop_id]
                    database_util.update_sql(sql, data)
            except Exception as err:
                print('thread_queue update_follow_count err:'+str(err))
        else:
            QUEUE_LOCK.release()
        time.sleep(1)

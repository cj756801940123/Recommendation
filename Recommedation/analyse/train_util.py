...
def train_snowNLP(table):
    ...
    get_sentiment_file(DATA_PATH + table + '/big_files/' + 'positive.txt',pos_file)
    get_sentiment_file(DATA_PATH + table + '/big_files/' + 'negative.txt',neg_file)
...
def get_sentiment_file(file_name,out_file):
    ...
    for each_line in file:
        index = each_line.find('comment:')
        if index >= 0:
            comment = each_line[index+8:].strip('\n')
            if len(comment)>10:
                print(comment)
